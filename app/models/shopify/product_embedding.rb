...
# Indexes
#
#  index_product_embeddings_on_account_id                           (account_id)
#  index_product_embeddings_on_shopify_product_id_and_content_hash  (shopify_product_id,content_hash) UNIQUE
#
module Shopify
  class ProductEmbedding < Shopify::ApplicationRecord
    ...
    validates :shopify_product_id, uniqueness: { scope: :content_hash }

    scope :for_account, ->(account_id) { where(account_id: account_id) }
    scope :with_embedding, -> { where.not(embedding: nil) }
    scope :recent, -> { order(embedded_at: :desc) }

    # Account bazlı semantic search

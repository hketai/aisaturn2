...
const documentTypeOptions = [
  { value: 'url', label: t('SATURN.DOCUMENTS.FORM.TYPE.URL') },
  { value: 'pdf', label: t('SATURN.DOCUMENTS.FORM.TYPE.PDF') },
];
...
const getErrorMessage = (field, errorKey) => {
  return v$.value[field].$error
    ? t(`SATURN.DOCUMENTS.FORM.${errorKey}.ERROR`)
    : '';
};
...
const handleFileChange = event => {
  ...
  if (file) {
    if (file.type !== 'application/pdf') {
      useAlert(t('SATURN.DOCUMENTS.FORM.PDF_FILE.INVALID_TYPE'));
      event.target.value = '';
      return;
    }
    if (file.size > MAX_FILE_SIZE) {
      useAlert(t('SATURN.DOCUMENTS.FORM.PDF_FILE.TOO_LARGE'));
      event.target.value = '';
      return;
    }
    state.pdfFile = file;
...
<template>
  <form class="flex flex-col gap-4" @submit.prevent="handleSubmit">
    <div class="flex flex-col gap-1">
      ...
      >
        {{ t('SATURN.DOCUMENTS.FORM.TYPE.LABEL') }}
      </label>
      <ComboBox
        id="documentType"
    ...
    <Input
      ...
      v-model="state.url"
      :label="t('SATURN.DOCUMENTS.FORM.URL.LABEL')"
      :placeholder="t('SATURN.DOCUMENTS.FORM.URL.PLACEHOLDER')"
    ...
    <div v-if="state.documentType === 'pdf'" class="flex flex-col gap-2">
      <label class="text-sm font-medium text-n-slate-12">
        {{ t('SATURN.DOCUMENTS.FORM.PDF_FILE.LABEL') }}
      </label>
      <div class="relative">
        <input
          ref="fileInputRef"
              ...
              <div class="flex flex-col flex-1 gap-1 items-start">
                <p class="m-0 text-sm font-medium text-n-slate-12">
                  {{
                    state.pdfFile
                      ? state.pdfFile.name
                      : t('SATURN.DOCUMENTS.FORM.PDF_FILE.CHOOSE_FILE')
                  }}
                </p>
                <p class="m-0 text-xs text-n-slate-11">
                  {{
                    state.pdfFile
                      ...
                      : t('SATURN.DOCUMENTS.FORM.PDF_FILE.HELP_TEXT')
                  }}
                </p>
              </div>
            </div>
    ...
    <Input
      v-model="state.name"
      :label="t('SATURN.DOCUMENTS.FORM.NAME.LABEL')"
      :placeholder="t('SATURN.DOCUMENTS.FORM.NAME.PLACEHOLDER')"
    />

    <div class="flex flex-col gap-1">
      <label for="assistant" class="mb-0.5 text-sm font-medium text-n-slate-12">
        {{ t('SATURN.DOCUMENTS.FORM.ASSISTANT.LABEL') }}
      </label>
      <ComboBox
        id="assistant"
        ...
        :placeholder="t('SATURN.DOCUMENTS.FORM.ASSISTANT.PLACEHOLDER')"
        class="[&>div>button]:bg-n-alpha-black2"

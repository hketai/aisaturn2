...
export const buildInboxData = inboxParams => {
  ...
  Object.keys(inboxProperties).forEach(key => {
    formData.append(`inbox[${key}]`, inboxProperties[key]);
  });
  ...
  if (selectedFeatureFlags) {
    if (selectedFeatureFlags.length) {
      selectedFeatureFlags.forEach(featureFlag => {
        formData.append(`inbox[channel][selected_feature_flags][]`, featureFlag);
      });
    } else {
      formData.append('inbox[channel][selected_feature_flags][]', '');
    }
  }
  Object.keys(channelParams).forEach(key => {
    formData.append(`inbox[channel][${key}]`, channel[key]);
  });
  return formData;
};

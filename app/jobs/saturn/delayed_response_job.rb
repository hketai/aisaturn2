...
class Saturn::DelayedResponseJob < ApplicationJob
  ...
  def generate_ai_response(message)
    ...
    # Son AI yanıtından sonraki TÜM mesajları topla
    pending_messages = collect_pending_messages
    Rails.logger.info "[SATURN DELAYED] Collected #{pending_messages.size} pending messages"

    # Multi-intent analizi yap
    @intent_result = analyze_intents(assistant, pending_messages)
    Rails.logger.info "[SATURN DELAYED] Detected intents: #{@intent_result[:intents].inspect}"

    # Check for intent-based handoff before processing
    if assistant.handoff_config.present? && assistant.handoff_config['enabled']
      # Handoff için human_request intent'i kontrol et
      if @intent_result[:intents].include?(:human_request)
      ...
      # Legacy intent detection (mevcut handoff config için)
      intent_detector = Saturn::IntentDetectionService.new(
        assistant: assistant,
        message_content: @intent_result[:combined_message]
      )
    ...
    # Chat service ile yanıt oluştur
    ...
    # Birleştirilmiş mesajı kullan (tüm pending mesajlar)
    ...
    Rails.logger.error("[SATURN DELAYED] AI response error: #{e.message}")
    Rails.logger.error(e.backtrace.join("\n")) if Rails.env.development?
  ...
  # Multi-intent analizi
  def analyze_intents(assistant, messages)
    message_contents = messages.map(&:content).reject(&:blank?)

    service = Saturn::MultiIntentDetectionService.new(
      assistant: assistant,
    ...
    Rails.logger.error "[SATURN DELAYED] Intent detection failed: #{e.message}"
    # Fallback
    {
      intents: [:other],
      product_keywords: [],
      combined_message: messages.map(&:content).join("\n"),
      raw_analysis: nil
    }
  end
